...
namespace VaccineC.Command.Application.Commands.Authorization
{
    public class UpdateAuthorizationCommand : IRequest<IEnumerable<AuthorizationViewModel>>
    {
        ...
        public UpdateAuthorizationCommand(Guid id, Guid userId, Guid eventId, Guid budgetProductId, Guid borrowerPersonId, Guid providerPersonId, int authorizationNumber, string situation, string typeOfService, string notify, DateTime authorizationDate, DateTime register)
        {
            ID = id;
            UserId = userId;
            EventId = eventId;
