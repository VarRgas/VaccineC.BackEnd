...
namespace VaccineC.Command.Application.Commands.MovementProduct
{
    public class AddMovementProductCommand : IRequest<IEnumerable<MovementProductViewModel>>
    {
        ...
        public AddMovementProductCommand(Guid id, Guid movementId, Guid productId, string? batch, decimal unitsNumber, decimal unitaryValue, decimal amount, string? details, DateTime register, DateTime? batchManufacturingDate, DateTime? batchExpirationDate, string? manufacturer)
        {
            ID = id;
